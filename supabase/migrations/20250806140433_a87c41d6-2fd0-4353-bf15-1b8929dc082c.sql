-- Delete existing characteristics for Escada and link new ones
WITH escada_flat AS (
  SELECT id FROM public.flats WHERE name ILIKE '%escada%' LIMIT 1
)
-- First delete existing characteristics
DELETE FROM public.flat_characteristics 
WHERE flat_id = (SELECT id FROM escada_flat);
...
FROM 
  (SELECT id FROM public.flats WHERE name ILIKE '%escada%' LIMIT 1) f,
  (VALUES 
